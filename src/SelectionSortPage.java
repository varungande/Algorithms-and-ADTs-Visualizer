/********************
 * Class Name: SelectionSortPage
 * Author: Varun Gande
 * Description: Controls the Graphics for the selection sort page
 *	     
 ********************/
...
public class SelectionSortPage {
	static int[] arr;
	static int size;
	static int y; 
	...
	/********************
	 * Constructor Name: SelectionSortPage
	 * Author: Varun Gande
	...
	public static Graphics paintScreen(Graphics g) {
		...
		g.drawString("Selection Sort", 630, 50);
		
		if (keepSorting) {
			// Selection Sort algorithm
			smallestIndex = j;
			for(int n = j; n < arr.length; n++) {
				if (arr[n] < arr[smallestIndex]) {
					smallestIndex = n;
				}
			}	
